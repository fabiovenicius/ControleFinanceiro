...
def trataData(data, parcela):
    ...
    if (parcela == 1):
        novaData = dataInformada
            ...
            novaDataIntermediaria = date.fromordinal(date.toordinal
(dataInformada) + qtdDiasAcrescentados)
            if (novaDataIntermediaria.day == 28):
                novaDataIntermediaria = date.fromordinal(date.toordinal
(dataInformada) + qtdDiasAcrescentados + 3)
...
novaDataIntermediaria.month, dia)
                ...
                novaDataIntermediaria = date.fromordinal(date.toordinal
(dataInformada) + qtdDiasAcrescentados + 2)
                novaData = date(novaDataIntermediaria.year,
novaDataIntermediaria.month, dia)
            elif (novaDataIntermediaria.day == 31):
                novaDataIntermediaria = date.fromordinal(date.toordinal
(dataInformada) + qtdDiasAcrescentados + 1)
                novaData = date(novaDataIntermediaria.year,
novaDataIntermediaria.month, dia)
            else:
                novaData = date(novaDataIntermediaria.year,
